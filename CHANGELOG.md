...
### Changed
- **BREAKING**: Output directory changed from `build/` to `dist/` (matching @arcaelas/rag structure)
- **BREAKING**: Binary name simplified to `arcaelas-mcp` only (removed `mcp` alias)
- Build script simplified: removed unnecessary `chmod +x` command
- Scripts updated to match proven working structure from @arcaelas/rag
- Fixed npx execution issues with published package

### Fixed
- Resolved "mcp: not found" error when using npx
- Package now works correctly with Claude Code and npx without global installation

## [1.2.1] - 2026-02-01

### Fixed
- Build script now automatically sets executable permissions on `build/index.js`
- Resolves "mcp: not found" error when running via npx from npm registry
- Binary now works correctly with Claude Code and other MCP clients
